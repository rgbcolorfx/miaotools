...
一个可部署到 GitHub Pages 的前端工具网站，目标是复刻 `19tools.com` 的多工具体验，并优先实现纯前端可运行能力。首页是清晰的工具目录，点击后进入单工具页面（URL 参数 `?tool=...`）。
...
- 生活计算(参考 66站可前端实现工具): 油耗计算、BMI、年龄计算、日期差、倒计时、百分比、单价计算
...
3. 推送后会自动执行 `.github/workflows/deploy.yml` 完成部署。
